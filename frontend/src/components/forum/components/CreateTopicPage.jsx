// src/components/forum/components/CreateTopicPage.jsx
...
const CreateTopicPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const newTopic = { subject, body, category };

    try {
      // Ganti URL '/api/topics' dengan endpoint API Anda yang sebenarnya
      ...
      const errorMessage = err.response?.data?.message || "Failed to publish topic. Please try again later.";
    ...
    <div className="min-h-screen bg-[#FBE8D3] pt-10 flex flex-col">
      <main className="flex-grow container mx-auto px-4 py-8">
        <div className="relative w-full h-62 md:h-68 overflow-hidden mb-8 shadow-lg">
          <img
            src="../img/banner_newtopik.svg" // <-- INI BAGIAN YANG DIPERBAIKI
          ...
          <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col items-center justify-center text-white text-center p-4">
            ...
            <p className="mt-2 text-sm max-w-xl">
              Start a new discussion — ask a question, share tips, or spark a conversation with fellow makers.
            </p>
          </div>
        </div>
        
        <form onSubmit={handleSubmit} className="max-w-4xl mx-auto bg-white/60 p-6 md:p-8 rounded-lg shadow-md border border-gray-200">
          <div className="mb-6">
            <label htmlFor="subject" className="block text-lg font-bold text-gray-800 mb-2">Subject</label>
            <input
              type="text"
              id="subject"
              value={subject}
          ...
          <div className="mb-6">
            <label className="block text-lg font-bold text-gray-800 mb-2">Body</label>
          ...
          <div className="mb-8">
            <label htmlFor="category" className="block text-lg font-bold text-gray-800 mb-2">Category</label>
            <select
              id="category"
              value={category}
              ...
              required
            >
              <option value="" disabled>Choose your topic's category</option>
